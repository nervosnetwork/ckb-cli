...
use ckb_sdk::constants::{
    DAO_TYPE_HASH, MULTISIG_SCRIPT, MULTISIG_SCRIPT_DEPRECATED, SIGHASH_TYPE_HASH,
};
...
impl Serialize for Script {
        ...
        S: Serializer,
    {
        ...
        //         String::from(" (sighash)")
        //     } else if self.code_hash == MULTISIG_DEPRECATED_TYPE_HASH {
        ...
        let code_hash_suffix = match self.hash_type {
            ScriptHashType::Type => match &self.code_hash {
                ...
                hash if hash == &MULTISIG_SCRIPT_DEPRECATED.code_hash => {
                    String::from(" (multisig_deprecated)")
                }
                hash if hash == &DAO_TYPE_HASH => String::from(" (dao)"),
                _ => String::new(),
            },
            ScriptHashType::Data1 => match &self.code_hash {
                hash if hash == &MULTISIG_SCRIPT.code_hash => String::from(" (multisig)"),
                _ => String::new(),
            },
            _ => String::new(),
        };
