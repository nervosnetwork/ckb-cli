...
fn snapshot_name() -> String {
    snapshot_name_at(Utc::now())
}

fn snapshot_name_at(now: DateTime<Utc>) -> String {
    // Multiple deployments can be applied within the same second. Keep the
    // timestamp fixed-width so lexicographic ordering still matches time order.
    now.format("%Y-%m-%d-%H%M%S%9f.json").to_string()
}

fn snapshot_name_regex() -> regex::Regex {
    regex::Regex::new(r"^\d{4}-\d{2}-\d{2}-\d{6}(?:\d{9})?\.json$").unwrap()
}
...
fn load_last_snapshot(migration_dir: &Path) -> Result<Option<DeploymentRecipe>> {
    // Accept legacy second-precision snapshots as well as the nanosecond-
    // precision names generated by current versions.
    let re = snapshot_name_regex();
...
#[cfg(test)]
mod tests {
    use super::{snapshot_name_at, snapshot_name_regex};
    use chrono::{TimeZone, Utc};

    #[test]
    fn snapshot_names_are_unique_and_sortable_within_a_second() {
        let first =
            Utc.with_ymd_and_hms(2026, 8, 6, 4, 39, 11).unwrap() + chrono::Duration::nanoseconds(1);
        let second = first + chrono::Duration::nanoseconds(1);

        let first_name = snapshot_name_at(first);
        let second_name = snapshot_name_at(second);

        assert_eq!(first_name, "2026-08-06-043911000000001.json");
        assert_eq!(second_name, "2026-08-06-043911000000002.json");
        assert!(first_name < second_name);
    }

    #[test]
    fn snapshot_name_regex_accepts_legacy_and_precise_names() {
        let re = snapshot_name_regex();
        assert!(re.is_match("2026-08-06-043911.json"));
        assert!(re.is_match("2026-08-06-043911000000001.json"));
        assert!(!re.is_match("2026-08-06-04391100000001.json"));
    }
}
